  ...
  if (error)
    return (
      <p className="text-red-500">
        {error.message || "Failed to load theater owners."}
      </p>
    );
    ...
    <div className="p-6 min-h-screen bg-gray-900 text-white">
      ...
      <div className="overflow-x-auto">
        <table className="table">
          ...
          <tbody>
            {theaterOwnerResponse?.map((theaterOwner, index) => (
              <tr key={theaterOwner._id}>
                {/* <th>
                  <label>
                    <input type="checkbox" className="checkbox" />
                  </label>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="avatar">
                      <div className="mask mask-squircle h-12 w-12">
                        <img
                          src={
                            theaterOwner.profilePic ||
                            "https://via.placeholder.com/150"
                          }
                          alt={theaterOwner.name}
                        />
                      </div>
                    </div>
                    <div>
                      <div className="font-bold">{theaterOwner.name}</div>
                    </div>
                  </div>
                </td>
                <td>{theaterOwner.email}</td>
                <td>{theaterOwner.mobile}</td>
                <th>
                  <button
                    onClick={() =>
                      navigate(
...
export default AllTheaterOwners;
